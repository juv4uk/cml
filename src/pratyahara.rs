    ...
    match expr {
        Expr::List(items) => {
            ...
            if let Expr::Symbol(op) = &folded[0] {
                ...
                if (op == "intersection" || op == "pratyahara-intersect" || op == "pratyahara-intersection" || op == "bit-and" || op == "and*") && args.len() == 2 {
                    if let (Some(m1), Some(m2)) = (resolve_mask_value(&args[0]), resolve_mask_value(&args[1])) {
                        return Expr::Integer((m1 & m2) as i64);
                    }
                }

                if (op == "union" || op == "pratyahara-union" || op == "bit-or" || op == "or*") && args.len() == 2 {
                    if let (Some(m1), Some(m2)) = (resolve_mask_value(&args[0]), resolve_mask_value(&args[1])) {
                        return Expr::Integer(((m1 | m2) & MASK_AL) as i64);
                    }
                }

                if (op == "diff" || op == "difference" || op == "pratyahara-diff" || op == "pratyahara-difference") && args.len() == 2 {
                    if let (Some(m1), Some(m2)) = (resolve_mask_value(&args[0]), resolve_mask_value(&args[1])) {
                        return Expr::Integer((m1 & (!m2) & MASK_AL) as i64);
                    }
                }

                if (op == "complement" || op == "pratyahara-complement" || op == "bit-not") && args.len() == 1 {
