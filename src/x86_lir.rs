...
use crate::machine_inst::{AluOp, CondCode, MachineInst, MachineItem, Provenance, X86Reg};
...
/// Maps virtual registers to physical x86-64 caller-saved registers for unoptimized baseline.
fn map_vreg_to_phys(vreg: VReg) -> Result<X86Reg, LirEmitError> {
    const ALLOCATABLE: [X86Reg; 9] = [
        X86Reg::Rax,
        X86Reg::Rcx,
        X86Reg::Rdx,
        X86Reg::Rsi,
    ...
    if idx < ALLOCATABLE.len() {
        Ok(ALLOCATABLE[idx])
    } else {
        Err(LirEmitError::RegisterExhaustion(format!(
            "virtual register {vreg} exceeds initial direct physical register pool; full linear-scan/graph-coloring lands in #56"
        )))
    }
}

/// Compiles an `LirFunction` CFG into a structured sequence of `MachineItem`s.
pub fn lir_to_machine_items(func: &LirFunction) -> Result<Vec<MachineItem>, LirEmitError> {
    let mut items = Vec::new();

    for block in &func.blocks {
        // Entry block does not need a label prefix if it's the start
        ...
        for inst in &block.instructions {
            match inst {
                ...
                LirInst::Alu {
                    ...
                    let phys_dst = map_vreg_to_phys(*dst)?;
                    let phys_lhs = map_vreg_to_phys(*lhs)?;
                    let phys_rhs = map_vreg_to_phys(*rhs)?;

                    if phys_dst != phys_lhs {
                        items.push(MachineItem::Inst(MachineInst::MovRegReg {
                            dst: phys_dst,
                            src: phys_lhs,
                            provenance: provenance.clone(),
        ...
        match &block.terminator {
            ...
            LirTerminator::Ret { val, provenance } => {
                if let Some(vreg) = val {
                    let phys_reg = map_vreg_to_phys(*vreg)?;
                    if phys_reg != X86Reg::Rax {
                        items.push(MachineItem::Inst(MachineInst::MovRegReg {
                            dst: X86Reg::Rax,
                            src: phys_reg,
                            provenance: provenance.clone(),
